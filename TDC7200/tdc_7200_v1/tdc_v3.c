...
//      UARTStdioConfig(0, 2000000 , SysCtlClockGet());
//
//
//
//
...
#define TDC7200_SPI_REG_ADDR_MASK               (0x1Fu)
#define TDC7200_SPI_AUTO_INC_MASK               (0x80u)
...
#define TDC7200_REG_SHIFT_CONFIG1_MEAS_MODE_1            (0x00u)
...
void spiReadReg2x24(const uint8_t addr, uint32_t *val1, uint32_t *val2);   //Setting up 2 byte
...
//    UARTprintf("Clock_Count1: '%x' \n",clock_count1);
    ...
    uint32_t calibration1=0, calibration2=0;
    spiReadReg2x24(TDC7200_REG_ADR_CALIBRATION1,&calibration1,&calibration2);
 //   UARTprintf("Calib 1 : '%x' \t",calibration1);
//    UARTprintf("Calib 2 f: '%x' \t",calibration2);
...
//    UARTprintf("t2 2 : '%x' \t",t2);
    ...
    tof = (((int64_t)(time1))*normLSB)>>shift;   //reg values
...
void spiReadReg2x24(const uint8_t addr, uint32_t *val1, uint32_t *val2)
{
    uint32_t temp=0;

        GPIOPinWrite(GPIO_PORTE_BASE, GPIO_PIN_0, 0);// Enabled SSI
        SSIDataPut(SSI2_BASE, ((addr & TDC7200_SPI_REG_ADDR_MASK)|TDC7200_SPI_REG_READ|TDC7200_SPI_AUTO_INC_MASK));
        SSIDataGet(SSI2_BASE, &junk);
        junk&= 0x00FF;
        SSIDataPut(SSI2_BASE,0u);
        SSIDataGet(SSI2_BASE, &temp);
